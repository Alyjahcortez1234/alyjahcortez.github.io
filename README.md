...
|      | A class that creates widgets that never change. Good for static screens. | `class HomeScreen extends StatelessWidget` | A stop sign always says “STOP.” |  |
|      | A class for widgets that can change while the app is running. | `class MyWidget extends StatefulWidget` |  A digital clock, a like button that toggles, or a shopping cart counter. |  |
...
|      | Makes space around a widget inside its container. | `Padding(padding: EdgeInsets.all(8.0), child: ...)` |  Like putting bubble wrap around an item in a box — it creates space so the item isn’t touching the edges. |  |
...
|      | Automatically puts widgets onto a new line when there's no space. | `Wrap(children: [...])` | Like words in a paragraph: when you reach the end of the line, the text automatically wraps to the next line. |  | 
...
|      | The special function in every widget that describes what gets drawn on the screen. | `Widget build(BuildContext context) {...}` |Like an architect’s blueprint — it shows exactly what should appear and how it’s arranged.  |  |
|      | Required in every widget class to describe what to show. | `build` | Like a blueprint an architect makes before constructing a building—it tells exactly what goes where. |  |
|      | A variable that helps the widget know where it is and lets it communicate with the app. | `BuildContext context` |Like a GPS or map in a city — it tells you exactly where you are so you can find nearby places or navigate.  |  |
|      | A keyword used to pass a value to the parent widget. | `super.key` | Like giving your home address to the postal service so they know exactly where to deliver your mail. |  |
|      | A keyword that means the value won't change and is set once. | `const` | Like a permanent stamp on a document — it cannot be changed once applied. |  |
